...
class Command(BaseCommand):
    ...
    def _create_events(self):
        ...
        ev1 = Event(name='Primavera Sound', state='SR', date=date1, description="Short Description", user=self.users[4])
        ev1.save()
        ev1.band.add(self.bands[1], self.bands[2])
        ev2 = Event(name='Acampada Jove', state='CL', date=date2, description="Short Description", user=self.users[5])
        ev2.save()
        ev2.band.add(self.bands[0], self.bands[3])
        ev3 = Event(name='Festiuet', state='SR', date=date3, description="Short Description", user=self.users[6])
        ev3.save()
        ev3.band.add(self.bands[0], self.bands[1])
        ev4 = Event(name='Ressurection fest', state='CL', date=date4, description="Short Description", user=self.users[7])
        ev4.save()
